...
parser.add_argument('--useLabelGen', action='store_true', help='Use a generator for label maps from chkpts')
...
def sample(sampler, useLabelGen=False):
    with torch.no_grad():
        samples = []
        for _ in tqdm(range(N // BS + 1)):
            if useLabelGen:
                x_real = sampler.sample_fromLabelGenerator(BS)[0].detach().cpu()
            # x_real = sampler.sample(BS)[0].detach().cpu()
            else:
                x_real = sampler.sample_fromDataset(BS)[0].detach().cpu()
        ...
        if path.endswith('.yaml'):
            ...
            for checkpoint in checkpoint_files:
                if (checkpoint.endswith('.pt') and checkpoint != 'model.pt') or checkpoint == 'pretrained':
                    ...
                    if 'label_gen' in checkpoint:
                        print("IGNORE LABEL GEN")
                        continue
